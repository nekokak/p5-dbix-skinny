...
=head2 single / search / search_named /search_by_sql
...
このようにスカラーコンテキストで受けた場合はDBIx::Skinny::Iteratorが取得でき、

    my @rows = Proj::Model->search('user',
        {
            name => 'nekokak',
        },
        { }
...
このようにリストコンテキストで受けた場合は結果Rowの配列を取得することができます。
